...
const GalleryTable: React.FC = observer(() => {
  const { imageStore, newsStore } = useStore();
  const { selectedNews } = newsStore;
  const [gallery, setGallery] = useState<GalleryImage[]>(selectedNews!.gallery!.galleryImages);

  const { deleteImage, setGalleryOrders, loading } = imageStore;
  const { message } = App.useApp();

  useEffect(() => {
    setGallery(selectedNews!.gallery!.galleryImages);
  }, [selectedNews]);
      ...
      render: (_, record) => (
        <Popconfirm
          ...
          onConfirm={() => {
            ...
            selectedNews!.gallery!.galleryImages = selectedNews!.gallery!.galleryImages.filter(
  ...
  const onDragEnd = ({ active, over }: DragEndEvent) => {
    if (active.id !== over?.id) {
      ...
      setGalleryOrders(newDataSource);
