...
const GalleryList = observer(() => {
  ...
  const { loadNewsById, clearSelectedNews, selectedNews } = newsStore;
  const [fileList, setFileList] = useState<UploadFile[]>([]);
  const { uploadImagesToGallery, loading } = imageStore;
  ...
  useEffect(() => {
    if (newsId) {
      loadNewsById(Number(newsId));
    }

    return () => clearSelectedNews();
  }, [newsId, loadNewsById, clearSelectedNews, selectedNews]);
  ...
  const handleUpload = async () => {
    if (newsId) {
      await uploadImagesToGallery(newsId, fileList);
      setFileList([]);
    }
  };

  if (!selectedNews) return <LoadingComponent />;
    ...
    <>
      <Breadcrumb
        ...
        items={[
          ...
          {
            title: <Link to="/haberler">Tüm Haberler</Link>,
          },
          {
            title: "Haber Galerisi" + (newsId ? ` (${newsId})` : ""),
          },
        ]}
      />
      <div>
      ...
      <div style={{ marginTop: "1rem" }}>{selectedNews?.gallery && <GalleryTable />}</div>
    </>
  );
});
